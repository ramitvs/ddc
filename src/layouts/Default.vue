<template>
    <div id="app">
        ...
        <header id="topnav" class="fixed flex flex-row cc:flex-col z-50 items-center w-full bg-white">
            <nav role="navigation" class="flex flex-col items-center space-between max-w-1200 mx-auto w-full px-6 py-0 cc:py-0">
                                    ...
                                    <transition name="slider">
                                        <ul v-show='toggle' id="submenu" class="cc:absolute py-3 whitespace-no-wrap bg-yellow1 text-white min-w-full cc:min-w-200 rounded-sm submenu" aria-label="submenu">
                                            <li class="main-nav-link" v-for="category in $page.allCategory.edges" :key="category.id">
                                                <a  v-text="category.node.title === 'guest-blog' ? 'Guest Blog' : category.node.title"  :href="`/categories/${category.node.title}`" aria-haspopup="true" class="flex px-8 py-2 cc:px-8 w-full"> {{ category.title }} </a>
                                            </li>
                                        </ul>
                                    </transition>
                                </label>
